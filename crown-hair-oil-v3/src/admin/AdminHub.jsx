...
import BankTransferReview from './BankTransferReview.jsx'

export default function AdminHub(){
  const params=new URLSearchParams(location.search)
  const requested=params.get('mode')
  const initial=requested==='commerce'?'commerce':requested==='payments'?'payments':requested==='contact'?'contact':requested==='bank-transfers'?'bank-transfers':'site'
  ...
  return <div className="admin-hub" dir="rtl">
    <nav className="admin-hub-nav" aria-label="أقسام لوحة الإدارة">
      ...
      <div className="admin-hub-switch"><button className={mode==='site'?'active':''} onClick={()=>change('site')}>تحرير الموقع</button><button className={mode==='contact'?'active':''} onClick={()=>change('contact')}>تواصل معنا</button><button className={mode==='commerce'?'active':''} onClick={()=>change('commerce')}>Commerce</button><button className={mode==='bank-transfers'?'active':''} onClick={()=>change('bank-transfers')}>التحويلات البنكية</button><button className={mode==='payments'?'active':''} onClick={()=>change('payments')}>إدارة طرق الدفع</button></div>
    </nav>
    <div className="admin-hub-body">{mode==='site'?<AdminVisualCMS/>:mode==='contact'?<ContactSettings/>:mode==='commerce'?<CommerceAdmin onOpenSite={()=>change('site')}/>:mode==='bank-transfers'?<BankTransferReview/>:<PaymentSettings/>}</div>
  </div>
}
