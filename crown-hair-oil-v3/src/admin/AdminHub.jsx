...
export default function AdminHub(){
  ...
  const initial=params.get('mode')==='commerce'?'commerce':params.get('mode')==='payments'?'payments':'site'
  ...
  return <div className="admin-hub" dir="rtl">
    <nav className="admin-hub-nav" aria-label="أقسام لوحة الإدارة">
      ...
      <div className="admin-hub-switch"><button className={mode==='site'?'active':''} onClick={()=>change('site')}>تحرير الموقع</button><button className={mode==='commerce'?'active':''} onClick={()=>change('commerce')}>Commerce</button><button className={mode==='payments'?'active':''} onClick={()=>change('payments')}>الدفع والتحويل البنكي</button></div>
    </nav>
    <div className="admin-hub-body">{mode==='site'?<AdminVisualCMS/>:mode==='commerce'?<CommerceAdmin onOpenSite={()=>change('site')}/>:<PaymentSettings/>}</div>
  </div>
}
